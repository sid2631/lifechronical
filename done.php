...
class IB_Stripe_Settings {
    ...
    function ib_stripe_donation_payment_processing_handler() {
        //var_dump($_POST);
        header("location: ".home_url().'/life-payment?value='.$_POST['value'].'&interval='.$_POST['interval']);
    }
    ...
    function ib_stripe_payment_processing_method(){
        ...
        if(isset( $_SESSION["donate_amount"]) && $_SESSION["donate_interval"] == 0 ){
            ...
            $product = array();
	           $product['id'] = "prod_KDYr9k0ocUFX1b";
	           //$product['id'] = "jhagbdasb";
            ...
            $data = self::ib_stripe_customer_create($user_wp_data,$card_data,$donate_key);
	 		...
	 		if(empty($data['api_error']) && $data['customer']){
	            ...
	            if( $subscription_data ){
	                ...
	                //code sid
	              /*  global $wpdb;
	                $table = $wpdb->prefix.'ib_stripe_order';
	                $data = array('user_id' => $user_wp_data->id,'subscription_id' => $subscription_data->id,'product_id' =>$product['id']);
	                $format = array('%s','%s','%s');
